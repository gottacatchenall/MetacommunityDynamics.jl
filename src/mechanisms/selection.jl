...
(gf::GaussianFitness)(dist::Number) = @fastmath exp(-1 * gf.α^2 * dist^2)

struct ExponentialFitness{LT} <: FitnessFunction
    λ::LT
end
...
function AbioticSelection{R,W}(; fitness::FitnessFunction = GaussianFitness()) where {R,W}
...
function BioticSelection{R,W}(; fitness::FitnessFunction = GaussianFitness()) where {R,W}
